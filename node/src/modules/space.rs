...
pub async fn new_space(db: &DatabaseConnection, dir: &str) -> Result<(), AppError> {
    ...
    if path.exists() {
            ...
            .await
        {
            Ok(Some(_existing_space)) => {
                info!("Space already exists at directory: {} (key: {})", dir, space_key);
                return Ok(());
            },
            Ok(None) => {
                warn!("Directory exists but no space record found. Creating space record for: {}", dir);
            },
            Err(e) => {
    ...
    match new_space.insert(db).await {
        ...
        Err(e) => {
            Err(AppError::Storage(Box::new(e)))
        }
    }
}
